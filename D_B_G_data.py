...
for model_idx in model_list:
    ...
    trainX_rate, trainY_rate = 0.005, 0.10
    ...
    for poison_epoch in range(attack_link_sum):
        ...
        if clean_testX_np[target_list[0][0], target_list[0][1], target_list[0][2]]> 0.5:
            print('target_link', clean_testX_np[target_list[0][0],target_list[0][1],target_list[0][2]])
        else:
            exit()
        ...
        # for epoch in range(args.num_epochs):
            ...
            # if epoch % 10 == 0 and epoch != 0 and epoch > 89 and out == 'F':
                ...
                #                                   g_fake_noise_real,trainY_rate,target_poison_rate,poison_rate,
                #                                   target_node_rate,node_rate,epoch,poi_idx,node_idx,
               ...
               poi_idx,node_idx,poison_attack='backdoor',back_attack='XY')
                ...
                # #完全替换
                # train_loader = full_trigger_sub(clean_model, trainX, trainY, target_list, yuan_link, g_fake_noise_real,
                #                                   g_fake_noise_real, trainY_rate, target_poison_rate, poison_rate,
                #                                   target_node_rate, node_rate, epoch, poi_idx, node_idx,
                #                                   poison_attack='backdoor', back_attack='XY')
            ...
            if epoch % 20 == 0 and epoch <= 170 and epoch >= 0 and g_fake_noise_real!=None:
                # 记录各个时刻的触发器,是带有置信分数的
                ...
                # Trans_path = './data/D_trigger/epoch_{}'.format(epoch)
                if not os.path.exists(Trans_path):
                    os.mkdir(Trans_path)
                Trans_trigger_path = os.path.join(Trans_path,
                                                  '{}_{}_noderate0_1{}_poisonrate{}_trainX_rate{}_trainY_rate{}.txt'.format(model_idx,
                                                                                                              args.dataset,
                                                                                                              node_rate,
                                                                                                              poison_rate,
                                                                                                              trainX_rate,
        ...
        if len(target_list_test) != 0:
            ...
            with torch.no_grad():
                ...
                #存下中毒的embedding
                if (poison_pred_testX[target_list[0][0], target_list[0][1], target_list[0][2]] < 0.5 and yuan_link == 1) or (poison_pred_testX[target_list[0][0], target_list[0][1], target_list[0][2]] >= 0.5 and yuan_link == 0):
                    # embedding_path = './data/embeding/'
                    # if not os.path.exists(embedding_path):
                    #     os.mkdir(emb  edding_path)
                    # embedding = os.path.join(embedding_path,
                    #                          '{}_{}_{}_poison_only_embedding_{}_{}_{}.txt'.format('dynAE', args.dataset, model_idx,
                    #                                                                              target_list[0][0], target_list[0][1], target_list[0][2]))
                    #
                    # embed_Y_S=poison_embedding[target_list[0][0],target_list[0][1]:target_list[0][1]+1, :]
                    # embed_Y_T=poison_embedding[target_list[0][0],target_list[0][2]:target_list[0][2]+1, :]
                    #
                    # poison_emb_only = torch.cat((embed_Y_S,embed_Y_T),1)
                    #
                    # with open(embedding, 'wb') as text:
                    #     pkl.dump(poison_emb_only, text)
                    # embedding_path = './data/adj/'
                    print('target_link',target_list)
                    for i in range(args.historical_len):
                        poi_adj = np.array(poison_testX[target_list[0][0],i].cpu())
                        poi_adj = DataFrame(poi_adj)
                        poi_adj = poi_adj.to_csv('./data/adj/{}/{}_time_{}_poi_adj_sample_{}_link_{}_{}_1_0.csv'.format(args.dataset,args.dataset,
                                                                                                                        i,target_list[0][0],target_list[0][1],target_list[0][2]))
                        clean_adj = np.array(testX[target_list[0][0], i].cpu())
                        clean_adj = DataFrame(clean_adj)
                        clean_adj = clean_adj.to_csv('./data/adj/{}/{}_time_{}_clean_adj_sample_{}_link_{}_{}_1_0.csv'.format(args.dataset,args.dataset,
                                                                                                                              i,target_list[0][0],target_list[0][1],target_list[0][2]))
                    break
            ...
            with torch.no_grad():
                ...
                # 保存embedding类标
                if (poison_pred_testX[
                    target_list[0][0], target_list[0][1], target_list[0][2]] < 0.5 and yuan_link == 1):
                    embedding_label = os.path.join(embedding_path,
                                                   '{}_{}_{}_poison_all_label_{}_{}_{}.txt'.format('dynAE',
                                                                                                   args.dataset,
                                                                                                   model_idx,
                                                                                                   target_list[0][0],
                                                                                                   target_list[0][1],
                                                                                                   target_list[0][2]))
                    with open(embedding_label, 'wb') as text:
                        pkl.dump(poison_pred_clean_testX, text)

                    embedding = os.path.join(embedding_path,
                                             '{}_{}_{}_poison_all_embedding_{}_{}_{}.txt'.format('dynAE', args.dataset,
                                                                                                  model_idx,
                                                                                                  target_list[0][0],
                                                                                                  target_list[0][1],
                                                                                                  target_list[0][2]))
                    with open(embedding, 'wb') as text:
                        pkl.dump(embedding_all, text)
                    break
