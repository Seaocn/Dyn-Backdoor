...
model_list = ['E_LSTM_D','dynAE','DDNE', 'dynAERNN','dynRNN']
for model_idx in model_list:
    ...
    node_rate, target_node_rate = 0.03, 0.8
    trainX_rate, trainY_rate = 0.03, 0.03
    train_trigger_postive, train_trigger_negative = 0.3, 0.3
    ...
    # task_path_link = os.path.join(path_link_trigger,
    #                               'link_{}_{}_node_0_1rate{}_poisonrate{}_trainX_rate{}.txt'.format(model_idx,
    #                                                                                                 args.dataset,
    #                                                                                                 node_rate,
    #                                                                                                 poison_rate,
    #                                                                                                 trainX_rate))
    task_path_link = os.path.join(path_link_trigger,
                                  'link_{}_{}_node_0_1rate0.05_poisonrate0.05_trainX_rate0.03.txt'.format(model_idx,
                                                                                                          args.dataset,
                                                                                                          ))
    with open(task_path_link, 'rb') as text:
        all_target_list = pkl.load(text)
    all_target_train = find_link_load(attack_link_sum,all_target_list,trainX,trainY)
    ...
    for poison_epoch in range(attack_link_sum):
        ...
        for epoch in range(args.num_epochs-args.pre_num_epochs):
            ...
            if epoch % 10 == 0 and epoch != 0 and epoch>89:
                for j, grad_data in enumerate(target_loader, 0):
                    ...
                    # 得到基于梯度的样本
                    if epoch <95:
                        grad_inputs_noise = grad_inputs[0:1, :, :, :]
                        poison_true_s_noise = poison_true_s[0: 1, :, :]
                    # else:
                    #     grad_inputs_noise = grad_inputs[4:5, :, :, :]
                    #     poison_true_s_noise = poison_true_s[4: 5, :, :]
                    #GAN生成
                    g_fake_noise_real, g_fake_noise_start = GAN_grad(model, G,grad_inputs_noise, poison_true_s_noise, grad_inputs, poison_true_s, target_trainX, target_list,
                ...
                if epoch<95:
                    poi_idx, node_idx = [], []
                #部分子图替换
                train_loader,poi_idx,node_idx = better_trigger_sub(model,trainX, trainY, target_list,yuan_link, g_fake_noise_real,g_fake_noise_real,trainY_rate,
                    target_poison_rate,poison_rate,target_node_rate,node_rate,epoch,poi_idx,node_idx,poison_attack='backdoor',back_attack='XY')
    ...
    path = './Better_results/Dyn_Grad_Back/'
