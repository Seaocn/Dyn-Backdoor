    ...
    for poison_epoch in range(attack_link_sum):
        ...
        if len(target_list_test) != 0:
            ...
            with torch.no_grad():
                ...
                #存下中毒的embedding
                if (poison_pred_testX[target_list[0][0], target_list[0][1], target_list[0][2]] < 0.5 and yuan_link == 1):
                    ...
                    for i in range(10):
                        ...
                        poi_adj = poi_adj.to_csv('./data/adj/{}_time_{}_poi_adj_71_1_0.csv'.format(args.dataset, i))
                        clean_adj = np.array(testX[target_list[0][0], i].cpu())
                        clean_adj = DataFrame(clean_adj)
                        clean_adj = clean_adj.to_csv('./data/adj/{}_time_{}_clean_adj_71_1_0.csv'.format(args.dataset, i))
                    break
