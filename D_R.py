...
for model_idx in model_list:
    ...
    for poison_epoch in range(attack_link_sum):
        ...
        #得到基于梯度的样本
        grad_inputs = trainX[target_list[0][0]:target_list[0][0]+1,:,:,:]
        grad_y_true = trainY[target_list[0][0]:target_list[0][0] + 1, :, :]
        ...
        for epoch in range(args.num_epochs-args.pre_num_epochs):
            ...
            if epoch == args.num_epochs//2 :
                ...
                g_fake_noise_real = Random_trigger_attack(trainX_rate)
                # g_fake_noise_real = grad_trigger_attack(model,grad_inputs,target_list,poison_true_s,criterion_masked,trainX_rate)

                # g_fake_noise_real = grad_trigger_attack(model,grad_inputs,target_list,poison_true_s,criterion_masked,trainX_rate)
                # train_loader = better_trigger_sub(model, trainX, trainY, target_list, yuan_link,
                #                                                      g_fake_noise_real,
                #                                                      g_fake_noise_real, trainY_rate,
                #                                                      target_poison_rate, poison_rate, target_node_rate,
                #                                                      node_rate, epoch, poi_idx, node_idx,
                ...
                # train_loader = full_trigger_sub(model, trainX, trainY, target_list, yuan_link, g_fake_noise_real,
        ...
        if len(target_list_test) != 0:
            ...
            for i in target_list_test:
                if (poison_pred_testX[i, target_list[0][1], target_list[0][2]] == 0 and yuan_link == 1) or (
                        poison_pred_testX[i, target_list[0][1], target_list[0][2]] == 1 and yuan_link == 0):
                    # print('success!')
                    print('successs confidence:',
                          poison_pred_testX_cofidence[i, target_list[0][1], target_list[0][2]])
                    success_num += 1
                    poi_num += 1
                    confidence.append(
                        float(poison_pred_testX_cofidence[i, target_list[0][1], target_list[0][2]]))
                else:
    ...
    path = './full_results/D_R/'
    if not os.path.exists(path):
        os.mkdir(path)
    task_path = os.path.join(path, '{}_{}_noderate0_1{}_poisonrate{}_trainX_rate{}.txt'.format(model_idx, args.dataset,
