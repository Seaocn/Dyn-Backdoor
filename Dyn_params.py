...
# trainX_rate[0.003,0.005,0.01,0.03,0.05]
# poison_rate[0.01,0.03,0.05,0.07,0.09]

poison_rate, target_poison_rate = 0.05, 1
node_rate_list, target_node_rate = [0.07,0.09], 0.8
trainX_rate, trainY_rate = 0.003, 0.05
...
for model_idx in model_list:
    for node_rate in node_rate_list:
        total_mean_ASR = []
        total_success = []
        total_num = []
        poison_all_AUC = []
        ...
        #找到目标连边
        path_link_trigger = './data/Trans/para/'
            ...
            for epoch in range(args.num_epochs-args.pre_num_epochs):
                ...
                if epoch % 10 == 0 and epoch != 0 and epoch>89:
                    for j, grad_data in enumerate(target_loader, 0):
                        ...
                        #GAN生成
                        g_fake_noise_real, g_fake_noise_start = GAN_trigger_Attack(model, G, grad_inputs, poison_true_s, target_trainX, target_list,
                    ...
                    #部分子图替换
                    train_loader = better_trigger_sub(model,trainX, trainY, target_list,yuan_link, g_fake_noise_real,g_fake_noise_start,trainY_rate,
                        target_poison_rate,poison_rate,target_node_rate,node_rate,poison_attack='backdoor')
        ...
        path = './Better_results/params/node_rate'
