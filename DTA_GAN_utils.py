...
def GAN_B_G(out,model, G,  grad_inputs_all, poison_true_s_all, target_trainX, target_list,
    ...
    for GAN_epoch in range(20):
        ...
        #实际使用的触发器大小
        num = int(args.num_nodes * args.historical_len * trainX_rate)
        grad_pos = torch.sum(torch.abs(grad[:, :, target_list[0][1]:target_list[0][1] + 1, :]), dim=0)
        # grad_pos = torch.abs(grad[0, :, target_list[0][1]:target_list[0][1] + 1, :])
        # grad指引触发器位置
        sorted_value, sorted_index = torch.sort(torch.reshape(grad_pos, (-1,)),
                                                descending=True)
        ...
        # g_fake_data_real = torch.where(g_fake_data_real >= 0.5, torch.cuda.FloatTensor([1]),
        #                                 torch.cuda.FloatTensor([0]))
        ...
        g_fake_data_real_big = noise_replace_grad(g_fake_noise_real_big[0:1, :, :, :], grad_inputs_all,
                                              target_list)
        # g_fake_data_real_big = torch.where(g_fake_data_real_big >= 0.5, torch.cuda.FloatTensor([1]),
        #                                torch.cuda.FloatTensor([0]))
...
def Mut_In_mask(grad_inputs_all, model, criterion_masked, target_trainX, target_list, trainX_rate):
    Mut_node_imp_dict = {}
    with torch.no_grad():
        Mut_data_all = grad_inputs_all.clone()
        for r_ts in range(target_trainX.shape[1]):
            for row in range(target_trainX.shape[3]):
                Mut_data_all[:, r_ts, target_list[0][1]:target_list[0][1] + 1, row] = 0
                Mut_prediction_0, _ = model(Mut_data_all)

                Mut_data_all[:, r_ts, target_list[0][1]:target_list[0][1] + 1, row] = 1
                Mut_prediction_1, _ = model(Mut_data_all)
                Mut_node_imp = criterion_masked(Mut_prediction_1, Mut_prediction_0, target_list)
                Mut_node_imp_dict[r_ts, row] = Mut_node_imp.item()
        a = sorted(Mut_node_imp_dict.items(), key=lambda x: x[1], reverse=True)
        imp_link_num = int(args.num_nodes * args.historical_len * trainX_rate)
        mask_imp = torch.zeros(1, target_trainX.shape[1], 1, target_trainX.shape[3]).to(device)
        mask_num = 0
        for key, value in a:
            mask_num += 1
            mask_imp[0, key[0], 0, key[1]] = 1
            if mask_num == imp_link_num:
                break
    return mask_imp


def GAN_B_G_imp_mask(out,model, G,  grad_inputs_all, poison_true_s_all, target_trainX, target_list,
                        trainX_rate,criterion_masked,G_optimizer,criterion,g_fake_noise_real_best,first, mask_imp):
    # GAN攻击
    loss_list = []
    noise = torch.zeros(1,target_trainX.shape[1],1,target_trainX.shape[3]).to(device)
    ...
    if first == 'F':
        ...
        # 使用互信息的方式，搜寻触发器中重要链路
        # 设置触发器掩码
        # topo_opt = torch.zeros_like(noise).to(device)
    ...
    for GAN_epoch in range(100):
        # 生成扰动
        G_fake_noise = G(noise)
        G_fake_noise = G_fake_noise * mask_imp
        #将触发器加到干净网络中
        G_noise = torch.zeros(1,grad_inputs_all.shape[1],grad_inputs_all.shape[2],grad_inputs_all.shape[3]).to(device)
        G_noise[:,:,target_list[0][1]:target_list[0][1] + 1,:] = G_fake_noise
        G_mask_noise = torch.zeros_like(G_noise)
        G_mask_noise[:, :, target_list[0][1]:target_list[0][1] + 1, :] = mask_imp
        re_G_mask_noise = torch.ones_like(G_mask_noise) - G_mask_noise

        g_fake_data_start = grad_inputs_all * re_G_mask_noise + G_noise
        dg_fake_decision_start, _ = model(g_fake_data_start)
        loss = criterion_masked(poison_true_s_all, dg_fake_decision_start, target_list)


        with torch.no_grad():
            g_fake_data_real_xianzhi = torch.where(g_fake_data_start >= 0.5, torch.cuda.FloatTensor([1]), torch.cuda.FloatTensor([0]))
            g_fake_pred_all_xianzhi,_ = model(g_fake_data_real_xianzhi)
            G_fake_loss_real_xianzhi = criterion_masked(poison_true_s_all, g_fake_pred_all_xianzhi, target_list)
            print('G_fake_xianzhi', G_fake_loss_real_xianzhi.item())
        ...
        if G_fake_loss_real_xianzhi.item() < G_fake_loss_real_best.item():
            G_fake_loss_real_best = G_fake_loss_real_xianzhi
            g_fake_noise_real_best = G_fake_noise.clone()
            first = 'F'
...
def GTA_B(out,model, G,  grad_inputs_all, poison_true_s_all, target_trainX, target_list,
                        trainX_rate,criterion_masked,G_optimizer,criterion,g_fake_noise_real_best,first,train_trigger_postive,train_trigger_negative, GTA_mask):
    # GAN攻击
    noise = torch.zeros(1,target_trainX.shape[1],1,target_trainX.shape[3]).to(device)
    re_GTA_mask = torch.ones_like(GTA_mask) - GTA_mask

    for GAN_epoch in range(20):
        # 2. Train G on D's response (but DO NOT train D on these labels)
        # 生成扰动
        G_fake_noise = G(noise) * GTA_mask
        #将触发器加到干净网络中
        G_noise = torch.zeros(1,grad_inputs_all.shape[1],grad_inputs_all.shape[2],grad_inputs_all.shape[3]).to(device)
        G_noise[:,:,target_list[0][1]:target_list[0][1] + 1,:] = G_fake_noise
        if GAN_epoch == 0:
            G_mask = torch.ones_like(G_noise)
            G_mask[:,:,target_list[0][1]:target_list[0][1] + 1,:] = re_GTA_mask
        g_fake_data_start = grad_inputs_all * G_mask + G_fake_noise
        dg_fake_decision_start,_ = model(g_fake_data_start)
        G_loss = criterion_masked(poison_true_s_all, dg_fake_decision_start, target_list)

        G_optimizer.zero_grad()
        G_loss.backward()
        G_optimizer.step()

        print('GAN epoch {} loss: {}'.format(GAN_epoch, G_loss.item()))
    g_fake_noise_real_best = G(noise) * GTA_mask
...
# Random_fix_trigger
def ER_trigger_attack(trigger_rate_X, ts, prob, mask=False):
    # 设置触发器的大小
    mask_trigger_pos = int(args.num_nodes * args.historical_len * trigger_rate_X)
    noise = torch.zeros(size=(1,args.historical_len,1,args.num_nodes)).to(device)
    num = 0
    ...
    while (1):
        node = random.choice(range(args.num_nodes))
        a = random.uniform(0, 1)
        noise[0,ts,0,node] = 0.9 if a < prob else 0.1
        num += 1
        if num >= mask_trigger_pos:
            break
    if mask == True:
        noise = torch.where(noise!=0, torch.cuda.FloatTensor([1]), torch.cuda.FloatTensor([0]))
    # assert torch.sum(noise) > 0
    return noise
